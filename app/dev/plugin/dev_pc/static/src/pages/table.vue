						...
						<mm_card>
							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list col="3">
										<mm_item>
											<control_input v-model="query.keyword" title="关键词" desc="表名 / 标题 / 描述 / 关键词"
											 @blur="search()" />
										</mm_item>
										<mm_item>
											<control_select v-model="query.app_id" title="适用的应用" :options="$to_kv(list_app, 'app_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
											<control_select v-model="query.group_id" title="数据表分组" :options="$to_kv(list_table_group, 'group_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
								...
								<mm_table type="2">
									<thead class="table-sm">
										<tr>
											...
											<th>
												<control_reverse title="适用的应用" v-model="query.orderby" field="app_id" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="数据表分组" v-model="query.orderby" field="group_id" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="使用次数" v-model="query.orderby" field="num" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="表名" v-model="query.orderby" field="name" :func="search"></control_reverse>
											</th>
											<th>
			...
			<mm_card class="card">
				...
				<div class="card_body">
					<dl>
						<dt>适用的应用</dt>
						<dd>
							<control_select v-model="form.app_id" :options="$to_kv(list_app, 'app_id', 'name')" />
						</dd>
						<dt>数据表分组</dt>
						<dd>
							<control_select v-model="form.group_id" :options="$to_kv(list_table_group, 'group_id', 'name')" />
						</dd>
					</dl>
				</div>
				<div class="card_foot">
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/dev/table",
				url_del: "/apis/dev/table?method=del&",
				url_set: "/apis/dev/table?method=set&",
				url_import: "/apis/dev/table?method=import&",
				url_export: "/apis/dev/table?method=export&",
				field: "table_id",
				query_set: {
					"table_id": ""
				},
