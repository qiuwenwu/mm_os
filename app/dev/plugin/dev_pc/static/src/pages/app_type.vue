							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list col="3">
										...
										<mm_item>
											<control_select v-model="query.father_id" title="上级分类" :options="$to_kv(list_app_type, 'type_id', 'name')"
											 @change="search()" />
										</mm_item>
										<mm_item>
			...
			<mm_card class="card">
				...
				<div class="card_body">
					<dl>
						<dt>上级分类</dt>
						<dd>
							<control_select v-model="form.father_id" :options="$to_kv(list_app_type, 'type_id', 'name')" />
						</dd>
					</dl>
				</div>
				<div class="card_foot">
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/dev/app_type",
				url_del: "/apis/dev/app_type?method=del&",
				url_set: "/apis/dev/app_type?method=set&",
				url_import: "/apis/dev/app_type?method=import&",
				url_export: "/apis/dev/app_type?method=export&",
				field: "type_id",
				query_set: {
					"type_id": ""
				},
