<template>
	<main id="stock_analyse_3_form">
		<mm_grid>
			<mm_col width="33">
				<mm_form class="card">
					<header class="arrow">
						<h5>{{ form[field] ? '修改' : '创建' }}创业板股票分析</h5>
					</header>
					<dl>
						<dt>当日时间</dt>
						<dd>
							<mm_time v-model="form.datetime" type="datetime-local" />
						</dd>
						<dt>人工得分</dt>
						<dd>
...
<script>
	...
	export default {
		...
		data() {
			return {
				url_submit: "/apis/stock/analyse_3?",
				url_get_obj: "/apis/stock/analyse_3?method=get_obj",
				field: "code",
				query: {
					"code": 0
				},
