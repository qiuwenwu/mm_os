				...
				<mm_view>
					...
					<mm_body>
						<mm_form class="mm_filter">
							...
							<mm_list col="3">
								<mm_col>
									<mm_select v-model="query.ok" title="阶段" :options="$to_kv(arr_ok)" @change="search()" />
								</mm_col>
								<mm_col>
						...
						<div class="mm_action">
							<h5><span>操作</span></h5>
							<div class="">
								<mm_btn class="btn_primary-x" url="./market_6_form">添加</mm_btn>
						...
						<mm_table type="2">
							...
							<tbody>
								<tr v-for="(o, idx) in list" :key="idx" :class="{'active': select == idx}" @click="selected(idx)">
									...
									<td>
										<span>{{arr_ok[o.ok] }}</span>
									</td>
									<td>
										<mm_btn class="btn_primary" :url="'./market_6_form?code=' + o[field]">修改</mm_btn>
			...
			<mm_view class="card bg_no">
				...
				<mm_body>
					<dl>
						<dt>阶段</dt>
						<dd>
							<mm_select v-model="form.ok" :options="$to_kv(arr_ok)" />
						</dd>
					</dl>
				</mm_body>
				<footer>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/stock/market_6",
				url_del: "/apis/stock/market_6?method=del&",
				url_set: "/apis/stock/market_6?method=set&",
				field: "code",
				query_set: {
					"code": ""
				},
