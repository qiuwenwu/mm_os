				...
				<mm_view>
					...
					<mm_body>
						<mm_form class="mm_filter">
							...
							<mm_list col="3">
								<mm_col>
									<mm_select v-model="query.user_id" title="分享者" :options="$to_kv(list_account, 'user_id', 'nickname')" @change="search()" />
								</mm_col>
								<mm_col>
						...
						<div class="mm_action">
							<h5><span>操作</span></h5>
							<div class="">
								<mm_btn class="btn_primary-x" url="./share_log_form">添加</mm_btn>
						...
						<mm_table type="2">
							<thead>
								<tr>
									...
									<th scope="col">
										<mm_reverse title="分享者" v-model="query.orderby" field="user_id" :func="search"></mm_reverse>
									</th>
									<th scope="col">
										<mm_reverse title="查询参数" v-model="query.orderby" field="query" :func="search"></mm_reverse>
									</th>
									<th scope="col">
										<mm_reverse title="访问源IP" v-model="query.orderby" field="ip" :func="search"></mm_reverse>
									</th>
							...
							<tbody>
								<tr v-for="(o, idx) in list" :key="idx" :class="{'active': select == idx}" @click="selected(idx)">
									...
									<td>
										<mm_btn class="btn_primary" :url="'./share_log_form?log_id=' + o[field]">修改</mm_btn>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/url/share_log",
				url_del: "/apis/url/share_log?method=del&",
				url_set: "/apis/url/share_log?method=set&",
				field: "log_id",
				query_set: {
					"log_id": ""
				},
