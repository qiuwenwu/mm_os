				...
				<mm_view>
					...
					<mm_body>
						<mm_form class="mm_filter">
							...
							<mm_list col="3">
								<mm_col>
									<mm_select v-model="query.user_id" title="创建者" :options="$to_kv(list_account, 'user_id', 'nickname')" @change="search()" />
								</mm_col>
								<mm_col>
						...
						<mm_table type="2">
							<thead>
								<tr>
									...
									<th scope="col">
										<mm_reverse title="创建者" v-model="query.orderby" field="user_id" :func="search"></mm_reverse>
									</th>
									<th scope="col">
										<mm_reverse title="路由键" v-model="query.orderby" field="key" :func="search"></mm_reverse>
									</th>
									<th scope="col">
										<mm_reverse title="转发网址" v-model="query.orderby" field="url_redirect" :func="search"></mm_reverse>
									</th>
									<th scope="col">
							...
							<tbody>
								<tr v-for="(o, idx) in list" :key="idx" :class="{'active': select == idx}" @click="selected(idx)">
									...
									<td>
										<mm_btn class="btn_primary" :url="'./share_form?share_id=' + o[field]">修改</mm_btn>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 列表请求地址
				url_get_list: "/apis/url/share",
				url_del: "/apis/url/share?method=del&",
				url_set: "/apis/url/share?method=set&",
				field: "share_id",
				query_set: {
					"share_id": ""
				},
